...
const DIGIT_BIG =
  "text-[4.5rem] sm:text-[8.5rem] leading-none font-bold tracking-tight text-white drop-shadow-[0_8px_40px_rgba(82,148,255,0.45)] tabular-nums";
const DIGIT_SMALL =
  "text-[2rem] sm:text-[4rem] leading-none font-bold tracking-tight text-white/85 drop-shadow-[0_8px_40px_rgba(82,148,255,0.45)] tabular-nums";
const SEP_BIG =
...
const Demo1 = () => {
  ...
  return (
    <main className={`${facultyGlyphic.className} main-content-1 min-h-screen flex flex-col relative overflow-hidden`}>
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-[58%] -translate-x-1/2 -translate-y-1/2 w-[80vw] max-w-[900px] h-[80vw] max-h-[900px] rounded-full blur-3xl opacity-60"
        style={{
          background:
            "radial-gradient(circle, rgba(82,148,255,0.35) 0%, rgba(54,125,244,0.12) 40%, transparent 70%)",
        }}
      />
      ...
      <section className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-6 pb-16 gap-6 sm:gap-8">
        ...
        >
          <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle,rgba(82,148,255,0.25),transparent_65%)] blur-2xl" />
          <svg
            className="relative w-44 sm:w-56 animate-spin-slow"
            viewBox="0 0 460 460"
          >
            <defs>
              <path
